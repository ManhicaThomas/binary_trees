...
/**
 * bst_remove_recursive - Removes a node form a binary search tree recursively.
 * @root: Pointer to the root node of the BST to remove a node from.
 * @node: Pointer to the current node in the BST.
 * @value: Value to remove form the BST.
 * Return: Pointer to the root node after deletiion.
 */
bst_t *bst_remove_recursive(bst_t *root, bst_t *node, int value)
{
	if (node != NULL)
 ...
 * @root: Pointer to the new root node of the BST to remove a node from.
 * @value: Value to remove in the BST.
 * Return: Pointer to the new root node after deletion.
 * Description: If the node to be deleted has two children, it is
